#!/usr/bin/env python3
"""
Monier-Williams Dictionary Parser - FINAL WORKING VERSION
Correctly parses the MW dictionary with proper regex for unclosed tags.
...
class MonierWilliamsParser:
    ...
    def parse_entry(self, entry_lines):
        ...
        # FIXED: Extract headword - tags are NOT closed, so match until next <
        headword_match = re.search(r'<k1>([^<]+)', first_line)
        ...
        # Accept any non-empty definition
        if not definition:
            return None
